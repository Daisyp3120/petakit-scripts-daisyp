...
config.viewFinalImage = true;
    ...
    %config.inputFolder = 'E:\LauraCooper\PetaKit5DfromSLD\tests\data\Inputs\sld';
fprintf('Processing files in folder: %s\n', config.inputFolder);
    ...
    elseif ~isempty(allTifFiles)
         ...
         for i = 1:length(allTifFiles)
            ...
            if ~isempty(regexp(filename, pattern, 'once'))
                tifFiles = [tifFiles; allTifFiles(i)]; 
            end
            endo
    ...
    % Add debugging output to check values
disp(['skewAngle: ', num2str(config.skewAngle)]);
disp(['pixelSizeZ: ', num2str(config.dz)]);
%end
%% -----------------------------------------------------------------------
...
function seriesResult = convertSeriesToTif(r, seriesIndex, sldFileName, config, psf_metadata)
    ...
    % Add checks:
if ~isfield(config, 'skewAngle') || isempty(config.skewAngle)
    error('config.skewAngle is not defined or is empty.');
end

if ~isfield(size_metadata, 'pixelSizeZ') || isempty(size_metadata.pixelSizeZ)
    error('size_metadata.pixelSizeZ is not defined or is empty.');
end
...
function runDeconDeskewPipeline(seriesResult, config)
 hasShownViewer = 0;
    ...
    %logDeconTimes(fullfile(seriesResult.currentSeriesPath,'tifs',config.resultDirName), seriesResult.currentSeriesFolder, deconTime)
%%end 
