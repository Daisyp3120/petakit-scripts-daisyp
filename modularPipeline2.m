function modularPipeline2(psfFolder, inputFolder)
% modularPipeline2('E:\Daisy\Lattice-lightsheet\PSF', 'E:\Daisy\Lattice-lightsheet\input')
% modularPipeline prompts the user for:
%% modularpipeline2('F:\RBC-time-series', 'F:\PSFs_zeiss_simulated_czi') 
...
% deskew-only), with z–axis padding applied during conversion and removed before merging.
    ...
    % Arrays to hold timing info for each image
    allPreprocTimes = [];
    allDeconTimes = [];
    allPostprocTimes = [];
    overallStart= [];
...
overallStart = tic;
    preprocStart = tic; % Start timing (PREPROCESSING)
...
if isempty(psfFiles)
    ...
    if ~isempty(sldFiles)
         ...
         for i = 1:length(sldFiles)
             ...
             [postprocStart, preprocTime, deconTime] = processSldFile(sldFullPath, config, psf_metadata,preprocStart);
         end
    elseif ~isempty(cziFiles)
        ...
        for i=1:length(cziFiles)
            ...
            [postprocStart, preprocTime, deconTime] = processSldFile(cziFullPath, config, psf_metadata,preprocStart);
        end
    elseif ~isempty(sldyFiles)
        ...
        for i=1:length(sldyFiles)
            ...
            [postprocStart, preprocTime, deconTime] = processSldFile(sldyFullPath, config, psf_metadata,preprocStart);
        end
        ...
        if ~isempty(tifFiles)
            ...
            if isempty(seriesResult)
                ...
                if strcmp(config.processingMode, 'decon+deskew') || strcmp(config.processingMode, 'both')
                    ...
                    if (abs(tif3D_metadata.pixelSizeZ - psf_metadata.pixelSizeZ)<1e-3)
                        [postprocStart, deconTime] = runDeconDeskewPipeline(seriesResult, config);
                    else
                ...
                if strcmp(config.processingMode, 'deskew-only') || strcmp(config.processingMode, 'both')
                    [postprocStart, deconTime] = runDecononlyPipeline(seriesResult, config);
                end
            ...
            for i = 1:length(allTifFiles)
                if ~endsWith(allTifFiles(i).name,ignoreSuffixes,"IgnoreCase",true)
                    ...
                    [postprocStart, preprocTime, deconTime] = processSldFile(filepath, config, psf_metadata,preprocStart);
                end
            end
        end
    else
...
allPreprocTimes(end+1) = preprocTime;
allDeconTimes(end+1) = deconTime;
       postprocTime = toc(postprocStart);
        fprintf('Postprocessing time: %.2f seconds\n', postprocTime);
        allPostprocTimes(end+1) = postprocTime;

      %%  fprintf('--- Finished processing file %s ---\n\n', allFiles(cziFile).name);


overallTime = toc(overallStart);

 % At end of batch, print summary stats:
fprintf(' Batch processing summary \n');
fprintf('Overall total time: %.2f seconds\n', overallTime);
fprintf('Preprocessing time: Mean = %.2f s, SD = %.2f s\n', mean(allPreprocTimes), std(allPreprocTimes));
fprintf('Deconvolution time: Mean = %.2f s, SD = %.2f s\n', mean(allDeconTimes), std(allDeconTimes));
fprintf('Postprocessing time: Mean = %.2f s, SD = %.2f s\n', mean(allPostprocTimes), std(allPostprocTimes));
...
function [postprocStart, preprocTime, deconTime] = processSldFile(sldFileName, config, psf_metadata, preprocStart)
    ...
    for S = 0:nSeries-1
        ...
        if strcmp(config.processingMode, 'decon-only')
                        size_metadata = getSizeMetadata(r,S);    
            if (abs(size_metadata.pixelSizeZ - psf_metadata.pixelSizeZ)<1e-3)
                [postprocStart, deconTime] = runDeconOnlyPipeline(seriesResult, config);
            else
    ...
    for T = 0:size_metadata.stackSizeT-1
        for C = 0:size_metadata.stackSizeC-1
            ...
            elseif endsWith(sldFileName, {'.tif', '.tiff'}, 'IgnoreCase', true)
                array=fullarray(:,:,(C+1):size_metadata.stackSizeC:size_metadata.stackSizeZ*(T+1));
            end
...
% Load and view
stack = parallelReadTiff(tifFileName);
showImageStackWithSlider(stack, config, seriesResult);
end
...
function [postprocStart, deconTime] = runDeconDeskewPipeline(seriesResult, config)
 hasShownViewer = 0;
    fprintf('Running deconvolution+deskew pipeline for series: %s\n', seriesResult.currentSeriesFolder);
   
    % Start timing the deconvolution (MIDPROCESSING) step
      deconStart = tic;
    ...
    % Stop timing and print result
        ...
        deconTime = toc(deconStart);
        fprintf('Deconvolution + Deskew time: %.2f seconds\n', deconTime);
        %allDeconTimes(end+1) = deconTime;
...
stack = zeros(info(1).Height, info(1).Width, numImages, 'uint16');

for k = 1:numImages
    stack(:,:,k) = imread(outputTiffFile, k);
end
% Only show the image once
if ~hasShownViewer && config.viewFinalImage
    showImageStackWithSlider(stack, config, seriesResult);
    hasShownViewer = true;
end
end

    function [postprocStart, deconTime] = runDeconOnlyPipeline(seriesResult, config)
 hasShownViewer = 0;
    fprintf('Running deconvolution+deskew pipeline for series: %s\n', seriesResult.currentSeriesFolder);
    ...
    % Stop timing and print result
        ...
        deconTime = toc(deconStart);
        fprintf('Deconvolution time: %.2f seconds\n', deconTime);
        %allDeconTimes(end+1) = deconTime;
...
postprocStart = tic;
 ...
 dataPath_exps = fullfile(seriesResult.tifDir, config.resultDirName);
    ...
    % % Remove z-padding from the decon+deskew results.
    % deconDSDir = fullfile(dataPath_exps, 'DS');
    removePaddingFromDir(dataPath_exps, config);
    
    % Merge the deconvolved+deskewed images.
    outputTiffFile = fullfile(config.inputFolder, [seriesResult.currentSeriesFolder, '_decon.tif']);
    if isfield(config,'skewDirection')
        paraMergeTiffFilesToMultiDimStack(dataPath_exps, outputTiffFile, seriesResult.pixelSizeX, seriesResult.deskewedZSpacing, seriesResult.frameInterval, config.skewDirection);
    else
        paraMergeTiffFilesToMultiDimStack(dataPath_exps, outputTiffFile, seriesResult.pixelSizeX, seriesResult.deskewedZSpacing, seriesResult.frameInterval);
    end
    
    outputTiffFileMax = fullfile(config.inputFolder, [seriesResult.currentSeriesFolder, '_decon_MAX.tif']);
    inputToMergeMax = fullfile(dataPath_exps, 'MIPs');
    if isfield(config,'skewDirection')
        paraMergeMaxToStack(inputToMergeMax, outputTiffFileMax, seriesResult.pixelSizeX, seriesResult.frameInterval, config.skewDirection);
    else
        paraMergeMaxToStack(inputToMergeMax, outputTiffFileMax, seriesResult.pixelSizeX, seriesResult.frameInterval);
...
% Show the 3D image with scroll bar
function showImageStackWithSlider(imgStack,config, seriesResult)
    % Check input
    if ndims(imgStack) ~= 3
        error('Input must be a 3D image stack.');
    end

    fig = figure('Name', '3D Image Viewer', 'NumberTitle', 'off');
    ax = axes('Parent', fig);
    sliceIndex = 1;
    imHandle = imshow(imgStack(:,:,sliceIndex), [], 'Parent', ax);

    % Add slider 
    uicontrol('Style', 'slider', 'Min', 1, 'Max', size(imgStack,3), ...
              'Value', sliceIndex, 'SliderStep', [1/(size(imgStack,3)-1) , 10/(size(imgStack,3)-1)], ...
              'Position', [150 20 300 20], ...
              'Callback', @(src,~) updateSlice(src, imHandle, imgStack));
    outputTiffFile = fullfile(config.inputFolder, [seriesResult.currentSeriesFolder, '_decondeskew.tif']);
end

function updateSlice(src, imHandle, imgStack)
        slice = round(get(src, 'Value'));
        set(imHandle, 'CData', imgStack(:,:,slice));
        drawnow;
    end

%% -----------------------------------------------------------------------
%% Local Function: runDeskewOnlyPipeline  
function postprocStart = runDeskewOnlyPipeline(seriesResult, config)
...
config.viewFinalImage = true;
    ...
    % Processing mode: Choose 'deskew-only', 'decon-only', 'decon+deskew', or 'both'.
    config.processingMode = 'decon-only';
    config.resultDirName = 'deconvolved';
% 
% config.dz
% 
% 
% config.xyPixelSize
% 
% config.z_edge_padding
% 
% config.ChannelPatterns
% 
% config.resultDirName
end
